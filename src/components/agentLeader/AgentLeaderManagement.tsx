...
function StatCard({
  ...
  return (
    ...
    >
      ...
      {sub && (
        <p className={`relative mt-2 text-sm ${c.sub}`}>
          {sub}
        </p>
      )}
    </div>
...
export default function AgentLeaderManagement() {
  ...
  const sortedLeaders = useMemo(() => {
    if (!sortField || !sortDir) return leaders;
  ...
  const handleDelete = async () => {
    ...
    try {
      await deleteUser(String(deletingLeader._id)).unwrap();
      toast.success("Agent Leader deleted successfully");
      setIsDeleteOpen(false);
      setDeletingLeader(null);
      refetch();
    } catch (err: any) {
      toast.error(err?.data?.message || "Failed to delete agent leader");
    }
  };
  ...
  return (
    <div className="space-y-6">
      {/* Page Header */}
      <PageHeader
        title="Agent Leader Management"
        description="Manage all agent leaders and monitor their activity"
        breadcrumbs={[
        ...
        action={<div className="flex items-center gap-2">
          <Link href="/admin/dashboard/agent-leader/trash">
            <Button variant="default" className="group hover:cursor-pointer border-rose-600 text-white bg-rose-700 hover:bg-rose-800 hover:shadow-xl hover:text-white duration-500 dark:text-white mt-2 cursor-pointer font-bold tracking-widest uppercase transition-colors disabled:opacity-60 hover:scale-105 ease-in-out">
            ...
            <TableBody>
              {isLoading ? (
                ...
                sortedLeaders.map((leader, index) => {
                  const status = leader.isActive ?? IsActive.INACTIVE;
                  return (
                    <TableRow
...
`}
                      ...
                      <TableCell>
                        <div className="flex gap-1.5 justify-end">
                          <Button
                            ...
                            className="h-8 w-8"
                            title="View details"
                            onClick={() => openDetailsDialog(leader)}
                          >
        ...
        {!isLoading && sortedLeaders.length > 0 && (
          <div className="px-4 py-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              leader{sortedLeaders.length !== 1 ? "s" : ""}
      ...
      {/* ── Modals ── */}
      {/* Uncomment once modals are created: */}
      {editingLeader && (
        <UpdateAgentLeaderModal
          open={isUpdateOpen}
