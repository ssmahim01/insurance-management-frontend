...
//   useEffect(() => { setPage(1); }, [searchTerm, agentFilter, statusFilter, genderFilter]);
...
//   const hasActiveFilters = agentFilter !== "all" || statusFilter !== "all" || genderFilter !== "all";
...
//       const agent = c.createdBy;
//       if (!agent) return false;
//       if (typeof agent === "string") return agent === agentFilter;
//       return agent._id === agentFilter;
//     });
//   }, [customers, agentFilter]);

//   // ── client-side sort ──
//   const sortedCustomers = useMemo(() => {
//     if (!sortField || !sortDir) return filteredCustomers;
...
//   const clearFilters    = () => { setAgentFilter("all"); setStatusFilter("all"); setGenderFilter("all"); };
...
//   // ── agent name helper (createdBy is the agent for customers) ──
...
//                     (a: IUser) => String(a._id) === agentFilter,
//                   )?.name || "Select agent"}
//             </span>
//           </SelectTrigger>
//           <SelectContent>
...
//             <span>
//               {genderFilter === "all" ? "All Genders" : GENDER_LABELS[genderFilter]}
//             </span>
//           </SelectTrigger>
//           <SelectContent>
          ...
          <SelectTrigger className="w-52 h-9 text-sm">
            <span className="flex items-center gap-2">
              ...
              {filterMode === "by_leader" && selectedLeaderId
                ? (leadersData?.data ?? []).find((l: IUser) => String(l._id) === selectedLeaderId)?.name || "Leader"
                : "All Leaders"}
            </span>
          </SelectTrigger>
          <SelectContent>
      ...
      <div className="bg-white dark:bg-slate-900 rounded-xl border border-slate-200 dark:border-slate-800 overflow-hidden">
        ...
        {!isLoading && sortedCustomers.length > 0 && (
          <div className="px-4 py-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
            ...
            {totalPage > 1 && (
              <p className="text-xs text-slate-500 dark:text-slate-400">Page {page} of {totalPage}</p>
            )}
          </div>
        )}
      </div>
