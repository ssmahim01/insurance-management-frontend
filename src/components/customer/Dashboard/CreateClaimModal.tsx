...
import { FileText, ImagePlus, X, Wallet } from "lucide-react";
...
const getPackageName = (sub?: ISubscription): string => {
  ...
  if (typeof sub.package === "object" && sub.package) {
    return sub.package.name ?? `Subscription #${String(sub._id).slice(-6)}`;
  }

  return `Subscription #${String(sub._id).slice(-6)}`;
};
...
export function CreateClaimModal({
  ...
  // Only let the customer file a claim against a subscription that's actually
  // active — filtered client-side since the parent fetches the full list.
  const eligibleSubscriptions = subscriptions
    .filter((s) => s.status === SubscriptionStatus.ACTIVE)
    .filter((s): s is ISubscription & { _id: string } => typeof s._id === "string");
  ...
  const selectedSubscription = eligibleSubscriptions.find(
    (s) => s._id === watchedSubscription,
  );
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 pt-1">
            ...
            >
              <SelectTrigger className="h-9 text-sm w-full">
                <span className="truncate">
                  {getPackageName(selectedSubscription)}
                </span>
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-1.5">
            ...
            <Select
              ...
              onValueChange={(v) =>
                setValue("claimTitle", v as ClaimTitle, {
                  shouldValidate: true,
                })
              }
            >
              <SelectTrigger className="h-9 text-sm w-full">
                <span className={watchedClaimTitle ? "" : "text-slate-400"}>
                  {watchedClaimTitle
                    ? CLAIM_TITLE_LABELS[watchedClaimTitle]
                    : "Select claim type"}
                </span>
              </SelectTrigger>
              <SelectContent>
          ...
          <div className="space-y-4">
                ...
                <SelectTrigger className="h-9 text-sm w-full">
                  <span className={watchedPaymentMethod ? "" : "text-slate-400"}>
                    {watchedPaymentMethod
                      ? PAYMENT_METHOD_LABELS[watchedPaymentMethod]
                      : "Select a method"}
                  </span>
                </SelectTrigger>
                <SelectContent>
            ...
            {/* ── bKash / Nagad ── */}
            {(watchedPaymentMethod === PaymentMethod.BKASH ||
              watchedPaymentMethod === PaymentMethod.NAGAD) && (
              <div className="space-y-1.5 animate-in fade-in slide-in-from-top-2 duration-300">
                ...
                <Input
                  ...
                  {...register("paymentInfo.mobileNumber")}
                />
                {errors.paymentInfo?.mobileNumber && (
                  <p className="text-xs text-red-400">
                    {errors.paymentInfo.mobileNumber.message}
                  </p>
                )}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 animate-in fade-in slide-in-from-top-2 duration-300">
                <div className="space-y-1.5">
                  ...
                  {errors.paymentInfo?.bankName && (
                    <p className="text-xs text-red-400">
                      {errors.paymentInfo.bankName.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.paymentInfo?.accountName && (
                    <p className="text-xs text-red-400">
                      {errors.paymentInfo.accountName.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  {errors.paymentInfo?.routingNumber && (
                    <p className="text-xs text-red-400">
                      {errors.paymentInfo.routingNumber.message}
                    </p>
                  )}
                </div>
                ...
                <div className="space-y-1.5 sm:col-span-2">
                  ...
                  {errors.paymentInfo?.branchName && (
                    <p className="text-xs text-red-400">
                      {errors.paymentInfo.branchName.message}
                    </p>
                  )}
                </div>
              </div>
          ...
          <div>
            ...
            <div className="grid grid-cols-3 sm:grid-cols-4 gap-3 mt-2">
              {previews.map((src, idx) => (
                <div
                  key={idx}
                  className="relative aspect-square rounded-lg overflow-hidden border border-slate-200 dark:border-slate-700 bg-slate-50 dark:bg-slate-800/30"
                >
          ...
          <Button
            type="submit"
            disabled={isLoading}
            className="group hover:cursor-pointer bg-linear-to-r from-indigo-600 to-blue-600 hover:from-indigo-700 hover:to-blue-700 text-white duration-300 font-bold tracking-widest uppercase transition-all w-full mt-2 hover:shadow-lg hover:-translate-y-0.5 active:scale-[0.98] disabled:opacity-60"
          >
            {isLoading ? (
