...
export function CustomerDashboardOverview() {
  ...
  const { data: subsData, isLoading: isSubsLoading } =
    useGetMySubscriptionsQuery({ limit: 50 });
  const { data: claimsData, isLoading: isClaimsLoading } = useGetAllClaimsQuery(
    { limit: 50 },
  );
          ...
          <div>
            ...
            <div className="space-y-4">
              {isSubsLoading ? (
                ...
                subscriptions.map((s) => (
                  <SubscriptionCard key={String(s._id)} subscription={s} />
                ))
              )}
            </div>
          </div>
          ...
          <div>
            ...
            <div className="space-y-4">
              {isClaimsLoading ? (
                ...
                claims.map((c: any) => (
                  <ClaimCard key={String(c._id)} claim={c} />
                ))
              )}
            </div>
          </div>
