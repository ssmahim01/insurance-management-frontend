...
const BASIC_PRICING: Partial<Record<PlanTypeKey, PlanRow[]>> = {
  single: [
    ...
    {
      label: "Hospital Coverage (BDT 1,000 daily, max 5 days per claim)",
      values: ["BDT 15,000"],
    },
...
const STANDARD_PRICING: Partial<Record<PlanTypeKey, PlanRow[]>> = {
  single: [
    ...
    {
      label: "Hospital Coverage (BDT 2,000 daily, max 5 days per claim)",
      values: ["BDT 30,000"],
    },
...
const SMART_PRICING: Partial<Record<PlanTypeKey, PlanRow[]>> = {
  single: [
    ...
    {
      label: "Hospital Coverage (BDT 2,500 daily, max 5 days per claim)",
      values: ["BDT 45,000"],
    },
    {
      label:
...
const TIER_360_PRICING: Partial<Record<PlanTypeKey, PlanRow[]>> = {
  single: [
    ...
    {
      label: "Hospital Coverage (BDT 4,000 daily, max 5 days per claim)",
      values: ["BDT 80,000"],
    },
    {
      label:
...
export default function ProductShowcase() {
  ...
  const availableTypes = PLAN_TYPES.filter((t) => PLAN_PRICING[planId][t.id]);
  ...
  const selectPlan = (id: PlanId) => {
    ...
    router.replace(`/products?plan=${id}`, {
      scroll: false,
    });
  };
  ...
  useEffect(() => {
    ...
    if (plan && ["basic", "plus", "standard", "smart", "360"].includes(plan)) {
            ...
            <div className="mt-10 overflow-hidden rounded-2xl border bg-white shadow-sm dark:border-slate-800 dark:bg-slate-900">
              <div className="overflow-x-auto">
                <table className="w-full min-w-100 border-collapse text-left text-sm">
                  ...
                  <tbody>
                    {rows.map((row, i) => {
                      const isPriceRow = typeof row.values[0] === "string";
                      return (
                        <tr
                          key={row.label}
                          className={cn(
        ...
        <section className="border-y bg-slate-100 py-12 dark:border-slate-800 dark:bg-slate-900">
          <div className="mx-auto max-w-5xl px-4 text-center sm:px-6 lg:px-8 space-x-0 sm:space-x-3">
            ...
            <Button className="mt-6 bg-[#125394] hover:bg-[#0d4883] px-5 py-5">
              <Link href="/product-terms-of-conditions">
                Read Our Product Terms
              </Link>
            </Button>
