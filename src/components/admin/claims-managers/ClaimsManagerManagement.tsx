...
import { CreateClaimsManagerModal } from "./CreateClaimsManager";
import { UpdateClaimsManagerModal } from "./UpdateClaimsManager";
import { ScrollArea, ScrollBar } from "@/components/ui/scroll-area";
import { ClaimsManagerDetailsModal } from "./ClaimsManagerDetailsModal";
...
function ClaimsManagerRowSkeleton() {
  return (
    <TableRow>
      <TableCell>
...
export default function ClaimsManagerManagement() {
  ...
  // ── API ──
  const { data, isLoading, refetch } = useGetAllClaimsManagersQuery({
  ...
  // ── derived ──
  const managers: IUser[] = useMemo(() => data?.data?.data ?? [], [data]);
  ...
  return (
    <div className="space-y-6">
      ...
      <PageHeader
        ...
        action={
          <div className="flex items-center gap-2">
            <Link href="/admin/dashboard/claims-managers/trash">
              <Button
                variant="default"
            ...
            <CreateClaimsManagerModal onSuccess={refetch} />
          </div>
        }
      />
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        ...
        {isLoading ? (
          ...
          <>
            ...
            />
            <StatCard
              label="Inactive Claims Managers"
            ...
            <Table className="min-w-275">
              <TableHeader className="sticky top-0 z-10">
                <TableRow className="border-none bg-linear-to-r *:text-white from-indigo-600 via-blue-600 to-cyan-600 hover:bg-transparent">
                  <SortableTh field="name" label="Claims Manager" />
              ...
              <TableBody>
                {isLoading ? (
                  Array.from({ length: 6 }).map((_, i) => (
                    <ClaimsManagerRowSkeleton key={i} />
                  ))
                    ...
                    <TableCell colSpan={8}>
                      <div className="flex flex-col items-center justify-center py-16 text-slate-400">
                        ...
                        {searchTerm || hasActiveFilters ? (
                          ...
                          <>
                            <p className="text-base font-medium">
                              No Claims Managers added yet
                            </p>
                            <p className="text-sm mt-1">
                              Click the Add Claims Manager button to get started
                            </p>
                          </>
                        )}
                      </div>
        ...
        {!isLoading && sortedManagers.length > 0 && (
          <div className="px-4 py-3 border-t border-slate-200 dark:border-slate-800 flex items-center justify-between">
            <p className="text-xs text-slate-500 dark:text-slate-400">
              ...
              Claims Manager{sortedManagers.length !== 1 ? "s" : ""}
      ...
      <AlertDialog open={isDeleteOpen} onOpenChange={setIsDeleteOpen}>
        <AlertDialogContent>
          <AlertDialogHeader>
            <AlertDialogTitle>Delete Claims Manager</AlertDialogTitle>
            <AlertDialogDescription>
          ...
          <div className="flex gap-2">
            ...
            <AlertDialogAction
              ...
              disabled={isDeleting}
              className="text-white hover:cursor-pointer bg-red-600 hover:bg-red-700 focus:ring-red-600"
            >
