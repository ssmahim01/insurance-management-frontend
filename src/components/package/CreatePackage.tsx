...
import { useForm, useFieldArray } from "react-hook-form";
import { toast } from "sonner";
import { Plus, Package, Trash2, PlusCircle, ImagePlus, X, Users, User } from "lucide-react";
...
const createPackageSchema = z.object({
  ...
  description:      z.string().min(10, "Description must be at least 10 characters"),
  coverageAmount:   z.preprocess((v) => Number(v), z.number().min(1, "Coverage amount is required")),
  // Single (1 person) vs Joint (2 persons) package
...
export function CreatePackageModal({ onSuccess }: CreatePackageModalProps) {
  ...
  const watchedIsActive = watch("isActive");
  const watchedIsJoint  = watch("isJoint");
          ...
          <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 pt-1">
            ...
            <div>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                ...
                {/* Package Type — Single vs Joint */}
                <div className="space-y-1.5 sm:col-span-2">
                  <Label htmlFor="p-joint" className="text-xs font-semibold tracking-widest uppercase">Package Type</Label>
                  <Select
                    value={watchedIsJoint ? "joint" : "single"}
                    onValueChange={(v) => setValue("isJoint", v === "joint")}
                  >
                    <SelectTrigger id="p-joint" className="h-9 text-sm">
                      <span className="flex items-center gap-2">
                        {watchedIsJoint ? <Users className="w-3.5 h-3.5" /> : <User className="w-3.5 h-3.5" />}
                        {watchedIsJoint ? "Joint (covers 2 persons)" : "Single (covers 1 person)"}
                      </span>
                    </SelectTrigger>
                    <SelectContent>
                      <SelectItem value="single">
                        <span className="flex items-center gap-2"><User className="w-3.5 h-3.5" /> Single (covers 1 person)</span>
                      </SelectItem>
                      <SelectItem value="joint">
                        <span className="flex items-center gap-2"><Users className="w-3.5 h-3.5" /> Joint (covers 2 persons)</span>
                      </SelectItem>
                    </SelectContent>
                  </Select>
                  <p className="text-[11px] text-[#96999A]">
                    Joint packages will ask for a 2nd person&apos;s info during subscription.
                  </p>
                </div>
