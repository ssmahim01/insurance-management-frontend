...
import { useForm, useFieldArray } from "react-hook-form";
import { toast } from "sonner";
import { Package, Trash2, PlusCircle, ImagePlus, X, Users, User } from "lucide-react";
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-5 pt-1">
          ...
          <div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              ...
              <div className="space-y-1.5 sm:col-span-2">
                <Label htmlFor="up-joint" className="text-xs font-semibold tracking-widest uppercase">Package Type</Label>
                <Select
                  value={watchedIsJoint ? "joint" : "single"}
                  onValueChange={(v) => setValue("isJoint", v === "joint")}
                >
                  <SelectTrigger id="up-joint" className="h-9 text-sm">
                    <span className="flex items-center gap-2">
