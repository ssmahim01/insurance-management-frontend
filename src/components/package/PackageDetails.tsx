...
// // Single neutral card style for all plans — type is distinguished by the small
// // badge dot/label instead of a loud full-card background.
...
// // Neutral stat card — icon carries a subtle tint, card itself stays muted/uniform.
...
//                   <StatBox label="Active Subs"    value={analytics.activeSubscriptions  ?? analytics.active   ?? 0} icon={TrendingUp} tint="text-blue-600" />
//                   <StatBox label="Pending Subs"   value={analytics.pendingSubscriptions ?? analytics.pending  ?? 0} icon={Clock}      tint="text-amber-600" />
//                 </div>
//               </div>
//               <Separator />
//             </>
...
export function PackageDetailsModal({ open, onOpenChange, item }: PackageDetailsModalProps) {
  ...
  const analytics    = (item as any).analytics;
  const featureImage = (item as any).featureImage as string | undefined;
